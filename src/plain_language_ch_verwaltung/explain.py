...
Not explainability of the four LLMs themselves - none of them expose model internals
(see README, "Why not 'real' model-internals explainability?"). This instead explains a
small proxy model trained on the model-agnostic diff-tag features already computed by
diffing.py, to answer: of the structural changes a simplification makes (jargon removed,
...
deterministic/formula-based - no LLM opinion (like the judge score) is used as either
input or output, so this stays independent of the models being evaluated.

With only 120 rows this is illustrative feature attribution, not a validated predictive
model - report it as a description of the collected sample, not a generalizable claim.
"""
...
def run() -> dict:
	...
	# Honest fit check, not a headline number - 120 rows is not enough to claim a
	# validated predictive model, only to describe the collected sample.
	...
	importance = sorted(
		zip(feature_cols, mean_abs_shap), key=lambda t: t[1], reverse=True
	)
...
def main() -> None:
	...
	print(f"Trained on {summary['n_rows']} rows. 5-fold CV R²: "
		f"{summary['cv_r2_mean']:.2f} ± {summary['cv_r2_std']:.2f} (small-N, illustrative only)")
